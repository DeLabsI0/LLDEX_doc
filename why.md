...
The client is trading with a counterparty that is not in direct control of the customer funds before and after the trade. Trade settlement is happening on an agreed date. To settle the trade, the client is sending the money to the broker account. The broker after getting the money from the client is sending the opposite currency/cryptocurrency/security to the client \(the transfer of opposite amount happens after client payment is finalized\). Both parties are taking a market risk before the trade settlement \(price of the instrument would change and the opposite counterparty would not be willing to settle\). Client is also taking settlement risk, because there is no guaranty that Broker after will send back the opposite currency/crypto currency/security after recipience of customer funds.
...
* Crypto Currencies are traded all around the world and are not related to a particular region or exchange \(much more similar to FX than to an Equity market that linked to a particular location/exchange/time zone\)
...
In FX there are probably just 10-15 counterparties in the world that are responsible for probably 80% of FX risk management being done\(/volume matching\), all others are just middlemen, for one simile reason = they are not able to get enough flow to make the profit from taking of market risk. Top FX traders are algorithmic market makers like JPM, XTX, UBS that are running low latency engines that are handling market risk. 
...
* give last look protection for market makers to allow risk-free liquidity brokerage \(pass liquidity from one place to the other and make immidiate risk fee profit with every trade\)
